 ...
 * @returns {*}
 */
export function changeKey(originalKey, newKey, obj)
{
 ...
 * @returns {*}
 */
export function changeKeyNested(originalKey, newKey, obj)
{
    if (Array.isArray(obj)) {
        var newArr = [];
        for (var i = 0; i < obj.length; i++) {
 ...
 */
export function collectToObj(params, fromObj, atleastOneRequired) {
    var result = {};
    var fieldNames = [];
    var details = [];
    ...
    for (var i=0; i<params.length; i++) {
        ...
        var param = {name: params[i].name, value: value, required: params[i].required,
            hasValue: Validations.hasValue(value), valid: !(params[i].required && !Validations.hasValue(value) )};
        result[params[i].name] = param;
        if (params[i].alias) result[params[i].alias] = param;
        if (!param.valid) {
            result['valid'] = false;
